...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const data = await fetchBlobJson('stake-daily.json');
    if (!data) {
      return res.status(404).json({ error: 'No data. Run /api/refresh-stake-daily first.' });
    }
    res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate=600');
    return res.status(200).json(data);
  } catch (error) {
    console.error('Error reading stake-daily blob:', error);
